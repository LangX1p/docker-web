DARI --platform=linux/amd64 ubuntu:22.04
...
Jalankan apt update -y && apt install -y dbus-x11 x11-utils x11-xserver-utils x11-apps
JALANKAN apt install software-properties-common -y
RUN add-apt-repository ppa:mozillateam/ppa -y
JALANKAN echo 'Paket: *' >> /etc/apt/preferences.d/mozilla-firefox
RUN echo 'Pin: release o=LP-PPA-mozillateam' >> /etc/apt/preferences.d/mozilla-firefox
RUN echo 'Pin-Priority: 1001' >> /etc/apt/preferences.d/mozilla-firefox
RUN echo 'Unattended-Upgrade::Allowed-Origins:: "LP-PPA-mozillateam:jammy";' | tee /etc/apt/apt.conf.d/51unattended-upgrades-firefox
Jalankan apt update -y && apt install -y firefox
Jalankan apt update -y && apt install -y xubuntu-icon-theme
JALANKAN touch /root/.Xauthority
UNGKAPAN 5901
EKSPOSISI 6080
